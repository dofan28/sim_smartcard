<?php
$limit = 5; // Jumlah siswa per halaman
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$search = isset($_GET['search']) ? trim($_GET['search']) : '';
$offset = ($page - 1) * $limit;

$transactionClasses = getAllTransactionClassJoinStudentPaginated($db, $limit, $offset, $search);
$totalTransactionClasses = getTotalTransactionClasses($db, $search);
$totalPages = ceil($totalTransactionClasses / $limit);
$no = $offset + 1;
        ...
        <div class="flex items-center gap-4 p-4 bg-white rounded-md shadow">
            ...
            <div class="relative inline-block text-left">
                ...
                <div id="dropdownMenu" class="absolute right-0 hidden mt-2 bg-white divide-y divide-gray-100 shadow-lg w-72 rounded-xl ring-1 ring-black/5">
                    <div class="p-1">
                        <!-- PDF Option -->
                        <a href="/transaction/class/print/pdf" class="flex items-center px-2 py-3 text-sm text-gray-700 transition-all duration-200 rounded-md hover:bg-red-200 group">
                        ...
                        <!-- Excel Option -->
                        <a href="/transaction/class/print/excel" class="flex items-center px-2 py-3 text-sm text-gray-700 transition-all duration-200 rounded-md hover:bg-green-200 group">
                        ...
                        <?php foreach ($transactionClasses as $transactionClass): ?>
                            <tr class="border-t hover:bg-gray-100">
                                ...
                                <td class="px-6 py-4 space-x-2 text-sm font-poppins">
                                    <a href="/transaction/transports/<?= $transactionClass['id'] ?>" class="px-4 py-2 text-white bg-green-500 rounded-md hover:bg-green-600">Detail</a>
                                    <a href="/transaction/transports/<?= $transactionClass['id'] ?>/edit" class="px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600">Edit</a>
                                    <a href="/transaction/transports/<?= $transactionClass['id'] ?>/delete" class="px-4 py-2 text-white bg-red-500 rounded-md hover:bg-red-600">Hapus</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else : ?>
                        ...
                        <div class="p-4 space-y-3">
                            ...
                            <div class="space-y-1 text-sm text-gray-600">
                                <div class="flex justify-between">
                                    <span>Tanggal:</span>
                                    <span><?= $transactionClass['date'] ?></span>
                                </div>
                                <div class="flex justify-between">
                                    <span>Waktu Masuk:</span>
                                    <span><?= $transactionClass['check_in'] ?></span>
                                </div>
                                <div class="flex justify-between">
                                    <span>Waktu Pulang:</span>
                                    <span><?= $transactionClass['check_out'] ?></span>
                                </div>
                            </div>

                            <!-- Tombol Aksi -->
                            <div class="flex flex-wrap gap-2 pt-2">
                                <a href="/transaction/transports/<?= $transactionClass['id'] ?>"
                                    class="px-4 py-2 text-sm text-white bg-green-500 rounded-md hover:bg-green-600">Detail</a>
                                <a href="/transaction/transports/<?= $transactionClass['id'] ?>/edit"
                                    class="px-4 py-2 text-sm text-white bg-blue-500 rounded-md hover:bg-blue-600">Edit</a>
                                <a href="/transaction/transports/<?= $transactionClass['id'] ?>/delete"
