...
$transactionTransports = getAllTransactionTransportJoinStudentPaginated($db, $limit, $offset, $search);
$totalTransactionTransports = getTotalTransactionTransports($db, $search);
$totalPages = ceil($totalTransactionTransports / $limit);
$no = $offset + 1;
...
<main class="container px-6 py-3 mx-auto mb-8">
    <!-- Header -->
    <header class="mb-8">
        <h1 class="text-2xl font-semibold tracking-normal text-blue-900 font-poppins">Mengelola Log Akses Transportasi</h1>
    </header>

    <!-- Aksi Cepat -->
    <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:justify-between sm:items-center">
        <div class="flex items-center gap-4 p-4 bg-white rounded-md shadow">
            <a href="/transaction/transports/create"
                class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white transition-all duration-300 rounded-md shadow bg-amber-500 hover:bg-amber-600 font-poppins">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
                Buat Log Akses Transportasi
            </a>
            <span class="w-1 h-6 bg-gray-400 rounded-md"></span>
            <div class="relative inline-block text-left">
                <button type="button" onclick="toggleDropdown()" class="inline-flex items-center px-4 py-2 text-sm text-white bg-blue-500 border border-gray-300 rounded-md font-poppins hover:bg-blue-600 ">
                    Cetak
                ...
                <!-- Dropdown Menu -->
                <div id="dropdownMenu" class="absolute right-0 hidden mt-2 bg-white divide-y divide-gray-100 shadow-lg w-72 rounded-xl ring-1 ring-black/5">
                    <div class="p-1">
                        <!-- PDF Option -->
                        <a href="/transaction/transport/print/pdf" class="flex items-center px-2 py-3 text-sm text-gray-700 transition-all duration-200 rounded-md hover:bg-red-200 group">
                            ...
                            <div>
                                <p class="text-xs font-semibold text-red-400 group-hover:text-red-600 font-poppins">Unduh PDF</p>
                                <p class="text-xs text-gray-400 mt-0.5 font-poppins">Simpan sebagai Dokumen PDF</p>
                            </div>
                        </a>
                        <!-- Excel Option -->
                        <a href="/transaction/transport/print/excel" class="flex items-center px-2 py-3 text-sm text-gray-700 transition-all duration-200 rounded-md hover:bg-green-200 group">
                            ...
                            <div>
                                <p class="text-xs font-semibold text-green-400 group-hover:text-green-600 font-poppins">Unduh Excel</p>
    ...
    <div class="overflow-hidden bg-white border rounded-md shadow">
        ...
        <div class="hidden md:block">
            <table class="min-w-full text-left border-collapse">
                ...
                <tbody>
                    <?php if (!empty($transactionTransports)): ?>
                        <?php foreach ($transactionTransports as $transactionTransport): ?>
                            <tr class="border-t hover:bg-gray-100">
                                <td class="px-6 py-4 text-sm font-poppins"><?= $transactionTransport['full_name'] ?></td>
                                <td class="px-6 py-4 text-sm font-poppins"><?= $transactionTransport['date'] ?></td>
                                <td class="px-6 py-4 text-sm font-poppins"><?= $transactionTransport['check_in'] ?></td>
                                <td class="px-6 py-4 text-sm font-poppins"><?= $transactionTransport['check_out'] ?></td>
                                <td class="px-6 py-4 space-x-2 text-sm font-poppins">
                                    <a href="/transaction/transports/<?= $transactionTransport['id'] ?>" class="px-4 py-2 text-white bg-green-500 rounded-md hover:bg-green-600">Detail</a>
                                    <a href="/transaction/transports/<?= $transactionTransport['id'] ?>/edit" class="px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600">Edit</a>
                                    <a href="/transaction/transports/<?= $transactionTransport['id'] ?>/delete" class="px-4 py-2 text-white bg-red-500 rounded-md hover:bg-red-600">Hapus</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else : ?>
                    ...
                    <?php foreach ($transactionTransports as $transactionTransport): ?>
                        <div class="p-4 space-y-3">
                            ...
                            <div class="font-medium font-poppins">
                                <?= $transactionTransport['full_name'] ?>
                            </div>
                            ...
                            <div class="space-y-1 text-sm text-gray-600">
                                <div class="flex justify-between">
                                    <span>Tanggal:</span>
                                    <span><?= $transactionTransport['date'] ?></span>
                                </div>
                                <div class="flex justify-between">
                                    <span>Waktu Masuk:</span>
                                    <span><?= $transactionTransport['check_in'] ?></span>
                                </div>
                                <div class="flex justify-between">
                                    <span>Waktu Pulang:</span>
                                    <span><?= $transactionTransport['check_out'] ?></span>
                                </div>
                            </div>

                            <!-- Tombol Aksi -->
                            <div class="flex flex-wrap gap-2 pt-2">
                                <a href="/transaction/transports/<?= $transactionTransport['id'] ?>"
                                    class="px-4 py-2 text-sm text-white bg-green-500 rounded-md hover:bg-green-600">Detail</a>
                                <a href="/transaction/transports/<?= $transactionTransport['id'] ?>/edit"
                                    class="px-4 py-2 text-sm text-white bg-blue-500 rounded-md hover:bg-blue-600">Edit</a>
                                <a href="/transaction/transports/<?= $transactionTransport['id'] ?>/delete"
