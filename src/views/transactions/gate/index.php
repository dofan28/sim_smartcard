...
$transactionTransports = getAllTransactionGateJoinStudents($db);
?>
...
<main class="container h-screen px-6 py-12 mx-auto">
    ...
    <div class="flex flex-col items-center justify-between mb-6 sm:flex-row">
        <a href="/transcation/gates/create"
                    ...
                    <?php foreach ($transactionTransports as $transactionTransport): ?>
                        <tr class="border-t hover:bg-gray-100">
                            ...
                            <td class="px-6 py-4 text-sm font-poppins">
                                <a href="/transaction/gates/<?= $transactionTransport['id'] ?>" class="px-4 py-2 text-white bg-green-500 rounded-md hover:bg-green-600">Detail</a>
                                <a href="/transaction/gates/<?= $transactionTransport['id'] ?>/edit" class="px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600">Edit</a>
                                <a href="/transaction/gates/<?= $transactionTransport['id'] ?>/delete" class="px-4 py-2 text-white bg-red-500 rounded-md hover:bg-red-600">Hapus</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else : ?>
